...
As these lines are being written, there is no straightforward way to restore a Moulti instance from files generated by the "Save" feature.
That being said, this short bash+python script should help:

```bash
#!/usr/bin/env bash
dir="${1:?dir required}"
python3 -c 'import sys, json, moulti.protocol as m; all((m.send_to_moulti(json.loads(open(filepath, "r").read())) for filepath in sys.argv[1:]))' "${dir}"/*.properties.json
for log in "${dir}"/*.contents.log; do
    moulti pass "$(jq -r .id "${log/.contents.log/.properties.json}")" --read-size=1048576 < "${log}"
done
```
